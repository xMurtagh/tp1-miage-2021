...
## Nom des étudiants de ce monôme/binôme 
...
* Disposer d'un PC d'au moins 6 Gio de RAM avec 10 Gio de disque disponible
* Disposer d'une version recente de VirtualBox
* disposer d'un compte Github par personne (ou un pour deux personnes si vous êtes en binôme);
* Forker sur Github le projet `bflorat/tp1-miage-2020` puis cloner votre dépôt Github dans votre workspace. Attention : *un seul fork par monôme/binôme*  ;
* Télécharger l'image VirtualBox de l'environnement de développement ici (et la tester avant le TP). Le login/mot de passe est : `tp`/`tp`.

# Déroulement du TP

NOTE: [Rappel sur Git] Trois dépôts sont ici utilisés: le dépot Github de l'enseignant (`bflorat/tp1-miage-2021`), le dépot Github du binôme (`<x>/tp1-miage-2021`), le dépot local sur le portable de l'un ou des deux étudiants du binôme.
...
*Où se trouve le code métier (voir la règle de gestion RG 1) ? Cette règle est-elle facilement testable par un test unitaire ?*
...
[NOTE]
====
* Les URL des endpoints sont renseignés dans le contrôleur via les annotation `@XYZMapping` 
